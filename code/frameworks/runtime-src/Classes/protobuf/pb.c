...
#define test_buffer(L,idx)  ((pb_Buffer*)PB_luaL_testudata(L,idx,PB_BUFFER))
#define check_slice(L,idx)  ((pb_Slice*)luaL_checkudata(L,idx,PB_SLICE))
#define test_slice(L,idx)   ((pb_Slice*)PB_luaL_testudata(L,idx,PB_SLICE))
...
#ifndef PB_luaL_newlib /* not LuaJIT 2.1 */
#define PB_luaL_newlib(L, name,l) (lua_newtable(L), luaL_register(L,name,l))

static lua_Integer PB_lua_tointegerx(lua_State* L, int idx, int* isint) {
    lua_Integer i = lua_tointeger(L, idx);
    if (isint) *isint = (i != 0 || lua_type(L, idx) == LUA_TNUMBER);
    return i;
}

static lua_Number PB_lua_tonumberx(lua_State* L, int idx, int* isnum) {
    lua_Number i = lua_tonumber(L, idx);
    if (isnum) *isnum = (i != 0 || lua_type(L, idx) == LUA_TNUMBER);
    return i;
}

static void* PB_luaL_testudata(lua_State* L, int idx, const char* type) {
...
static int Lpb_delete(lua_State* L) {
    lpb_State* LS = (lpb_State*)PB_luaL_testudata(L, 1, PB_STATE);
    if (LS != NULL) {
...
static uint64_t lpb_tointegerx(lua_State* L, int idx, int* isint) {
    ...
    const char* s, * os;
#if LUA_VERSION_NUM >= 503
    uint64_t v = (uint64_t)PB_lua_tointegerx(L, idx, isint);
    if (*isint) return v;
#else
    uint64_t v = 0;
    lua_Number nv = PB_lua_tonumberx(L, idx, isint);
    if (*isint) {
...
static int lpb_addtype(lua_State* L, pb_Buffer* b, int idx, int type, size_t* plen) {
    ...
    case PB_Tbool:
        ...
        break;
    case PB_Tdouble:
        v.lnum = PB_lua_tonumberx(L, idx, &ret);
...
static const pb_Field* lpb_field(lua_State* L, int idx, const pb_Type* t) {
    ...
    int isint, number = (int)PB_lua_tointegerx(L, idx, &isint);
